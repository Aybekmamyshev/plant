...
import {getNewProducts} from "../../../../../redux/reducers/top";

const CartNew = () => {
    const {data} = useSelector((store) => store.top)
    const dispatch = useDispatch()
    useEffect(() => {
        axios('http://localhost:8080/products')
            .then(({data}) => dispatch(getNewProducts(data)))
            .catch((err) => err)
        ...
        <>
            {
                data.map((item) => (
                     <div className="cartNew">
                        <div className={'cartNew__wrapper'}>
                            <div className="cartNew__new">
                                New
                            </div>
                            <div className="cartNew__pic">
                                <img className={'cartNew__img'} src={item.image} alt=""/>
                                <div className="cartNew__icons">
                                    <div className="cartNew__basket">
                            ...
                            <h2 className="cartNew__title">{item.title}</h2>
                            <p className="cartNew__price">{item.price}</p>
                        </div>
                    </div>
                ))
