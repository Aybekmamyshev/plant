...
import "./cartsold.scss"
import {useDispatch, useSelector} from "react-redux";
import axios from "axios";
import {getSold} from "../../../../../redux/reducers/sold";

const CartSold = () => {
    const dispatch = useDispatch()
    const {data} = useSelector((store) => store.sold)
    useEffect(() => {
        axios('http://localhost:8080/products')
            .then(({data})=> dispatch(getSold(data)) )
            .catch((err) => err)
    },[])
    return (
           ...
           <>
               {
                   data.map((item) => (
                       <div key={item.id} className={'cartSold'}>
                           <div className={'cartSold__wrapper'}>
                               <div className="cartSold__new">
                                   Sold
                               </div>
                               <div className="cartSold__pic">
                                   <img className={'cartSold__img'} src={item.image} alt=""/>
                                   <div className="cartSold__icons">
                                       <div className="cartSold__basket">
                                       ...
                                       <div className="cartSold__search">
                                           <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                               <path
                               ...
                               <h2 className="cartSold__title">{item.title}</h2>
                               <p className="cartSold__price">{item.price}</p>
                           </div>
                       </div>
                   ))
               }
